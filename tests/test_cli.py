...
class FakePipelineNoLLM(FakePipeline):
    def __init__(self):
        self.context = type("Context", (), {"llm": None})()
...
class CLITests(unittest.TestCase):
    ...
    def test_llm_check_command_succeeds_when_llm_is_configured(self) -> None:
        stdout = io.StringIO()
        with patch("iirs.cli.IIRSPipeline", return_value=FakePipeline()):
            with redirect_stdout(stdout):
                rc = main(["llm-check"])
    ...
    def test_llm_check_command_fails_cleanly_when_llm_is_disabled(self) -> None:
        stdout = io.StringIO()
        with patch("iirs.cli.IIRSPipeline", return_value=FakePipelineNoLLM()):
            with redirect_stdout(stdout):
                rc = main(["llm-check"])
